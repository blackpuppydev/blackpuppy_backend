...
exports.getDisplayNext = async (req, res) => {
  try {
    const { data, error } = await supabase
      .from("customer_next")
      .select("id, name, price, place, type, date, province, link_drive")
...
exports.addCustomerNext = async (req, res) => {
  try {
    const { name, price, place, type, date, province, link_drive } = req.body;
    ...
    // เพิ่มข้อมูลลงใน Supabase
    const { data, error } = await supabase.from("customer_next").insert([
      {
        name,
        price,
        place,
    ...
    console.error("Add customer next error:", error);
    res.status(500).json({ message: error.message || "Internal Server Error" });
  }
};


exports.updateCustomerNext = async (req, res) => {
  try {
    const customerId = req.params.id;
    const { name, price, place, type, date, province, link_drive } = req.body;
...
exports.deleteCustomerNext = async (req, res) => {
  try {
    const customerId = req.params.id;
